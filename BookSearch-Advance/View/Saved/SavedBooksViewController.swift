...
final class SavedBooksViewController: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        viewModel.fetchBooks() // CoreData fetch는 ViewModel이 담당
    }
    ...
    @objc private func deleteAllBooks() {
        viewModel.deleteAllBooks() // ViewModel에서 CoreData 처리
    }
...
extension SavedBooksViewController: UITableViewDataSource, UITableViewDelegate {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        return viewModel.savedBooks.count // ViewModel 데이터 사용
    }
    
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "Cell", for: indexPath)
        cell.textLabel?.text = viewModel.savedBooks[indexPath.row].title
        return cell
    }
    ...
    -> UISwipeActionsConfiguration? {
        ...
        let deleteAction = UIContextualAction(style: .destructive, title: "삭제") { _, _, completion in
            self.viewModel.deleteBook(at: indexPath.row) // ViewModel에 위임
